...
void print(FILE *file, Scoreboarding scoreboarding, Instruction instructions[], RegisterMemory registers[32], int pc, int clock)
{
    ...
    for (int i = 8; i < 25; i++)
    {
        fprintf(file, "%s | ", registers[i].functionalUnityName);
    }
    fprintf(file, "%s\n", registers[25].functionalUnityName);

    fprintf(file, "\n4) BANCO DE REGISTRADORES\n");
    fprintf(file, "t0 | t1 | t2 | t3 | t4 | t5 | t6 | t7 | s0 | s1 | s2 | s3 | s4 | s5 | s6 | s7 | t8 | t9\n");
    for (int i = 8; i < 25; i++)
    {
        fprintf(file, "%i | ", registers[i].value);
    }
    fprintf(file, "%i\n", registers[25].value);
}
